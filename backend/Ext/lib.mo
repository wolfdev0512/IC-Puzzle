...
module {
    ...
    public class make (state : Types.State) {
        ...
        public func balance(canister: Principal, request: Ext.Core.BalanceRequest): Ext.Core.BalanceResponse {
        ...
        public func transfer(caller: Principal, request: Ext.Core.TransferRequest): async Ext.Core.TransferResponse {
            let index = switch (Ext.TokenIdentifier.decode(request.token)) {
                case (#err(_)) { return #err(#InvalidToken(request.token)); };
        ...
        public func metadata(caller: Principal, tokenId: Ext.TokenIdentifier): Ext.Common.MetadataResponse {
        ...
        public func bearer(tokenId: Ext.TokenIdentifier): Ext.NonFungible.BearerResponse {
            let index = switch (Ext.TokenIdentifier.decode(tokenId)) {
                case (#err(_)) { return #err(#InvalidToken(tokenId)); };
                case (#ok(_, tokenIndex)) { tokenIndex; };
            };
        ...
        public func allowance(caller: Principal, request: Ext.Allowance.Request): Ext.Allowance.Response {
            #err(#Other("disabled"));
        };

        public func approve(caller: Principal, request: Ext.Allowance.ApproveRequest) : ()
        {};
